...
use std::env;
use chrono::{NaiveDate, Local, Duration, Utc, DateTime};
...
async fn get_social_feed(pool: web::Data<PgPool>) -> impl Responder {
    let rows = sqlx::query_as::<_, (Uuid, String, String, String, Option<String>, Option<String>, String, i64, i64)>(
        ...
        SELECT 
            p.id,
            COALESCE(u.name, 'Administrador') as author_name,
            p.team_name,
            p.content,
            p.image_url,
            p.video_url,
...
async fn create_post(req: web::Json<CreatePostRequest>, pool: web::Data<PgPool>) -> impl Responder {
    ...
    let admin_id = match sqlx::query_scalar::<_, Uuid>(
        "SELECT id FROM users WHERE email = 'admin@retesp.com'"
    )
    .fetch_optional(&**pool)
    .await 
    {
        ...
        Ok(None) => {
            ...
            .bind(new_id)
            .bind("Administrador")
            .bind("admin@retesp.com")
            .bind("$2b$12$7uTkA8VKjLg5Q1OZzTZ9qO7kR8LmN9pQrS5tUwXyZ6aBcDeFgH4iJ")
            .bind("admin")
            .execute(&**pool)
            .await;
            new_id
        ...
        Err(e) => {
            eprintln!("Erro ao buscar usuário admin: {}", e);
            return HttpResponse::InternalServerError().body(format!("Erro ao buscar usuário admin: {}", e));
        }
    };
    ...
    .await {
        ...
        Err(e) => {
            ...
            HttpResponse::InternalServerError().body(format!("Erro ao criar postagem: {}", e))
        }
    }
}
...
async fn list_stories(pool: web::Data<PgPool>) -> impl Responder {
    let now = Utc::now();
    
    let rows = sqlx::query_as::<_, (Uuid, String, Option<String>, Option<String>, String, bool, DateTime<Utc>, DateTime<Utc>)>(
        "SELECT id, username, image, video_url, story_type, is_from_retesp, expires_at, created_at 
         FROM stories 
         WHERE expires_at > $1 
    ...
    match rows {
        Ok(rows) => {
            let stories: Vec<serde_json::Value> = rows.iter().map(|r| {
                serde_json::json!({
                    "id": r.0,
                    "user": r.1,
                    "image": r.2,
                    ...
                    "type": r.4,
                    "isFromRETESP": r.5,
                    "expiresAt": r.6.timestamp_millis(),
                    "created_at": r.7,
                })
            }).collect();
        ...
        Err(e) => {
            eprintln!("Erro ao buscar stories: {}", e);
            HttpResponse::InternalServerError().body("Erro ao buscar stories")
        }
    }
}

// Criar novo story
async fn create_story(req: web::Json<CreateStoryRequest>, pool: web::Data<PgPool>) -> impl Responder {
    let id = Uuid::new_v4();
    let now = Utc::now();
    ...
    let is_from_retesp = req.is_from_retesp.unwrap_or(false);

    match sqlx::query(
        "INSERT INTO stories (id, username, image, video_url, story_type, is_from_retesp, expires_at, created_at) 
    ...
    .await {
        Ok(_) => {
            HttpResponse::Created().json(serde_json::json!({
                ...
                "type": req.story_type,
                "isFromRETESP": is_from_retesp,
                "expiresAt": expires_at.timestamp_millis(),
                "created_at": now,
            }))
        },
        Err(e) => {
            eprintln!("Erro ao criar story: {}", e);
            HttpResponse::InternalServerError().body("Erro ao criar story")
        }
    }
}

// Deletar stories expirados
async fn delete_expired_stories(pool: web::Data<PgPool>) -> impl Responder {
    let now = Utc::now();
    
    match sqlx::query("DELETE FROM stories WHERE expires_at < $1")
        .bind(now)
        .execute(&**pool)
        .await {
            Ok(deleted) => {
                HttpResponse::Ok().json(serde_json::json!({
                    "message": format!("{} stories expirados removidos", deleted.rows_affected())
                }))
            },
            Err(e) => {
                eprintln!("Erro ao deletar stories expirados: {}", e);
                HttpResponse::InternalServerError().body("Erro ao deletar stories expirados")
            }
        }
}
